...
#include <pcl-1.8/pcl/filters/approximate_voxel_grid.h>   //该过滤器可以换成其他过滤器，但我发现近似体素过滤器可以产生最佳效果
...
public:
    CloudMapEva(Param &param)
            : param_(param), map_3d_(new PointCloud), gt_3d_(new PointCloud), processed_points(9) {       // processed_points=9原子变量初始化
        param_.printParam();   //打印参数
...
private:
    ...
    bool eva_mesh = false;                       //if we construct a simple mesh
