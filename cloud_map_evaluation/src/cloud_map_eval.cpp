...
int CloudMapEva::process() {
    TicToc tic_toc;   //时间函数
    io::ReadPointCloudOption io_params("auto", true, true, true);   //移除nan点和无效点等
    // Load point clouds
    io::ReadPointCloudFromPCD(param_.evaluation_map_pcd_path_ + "map.pcd", *map_3d_, io_params);   //!读取地图估计点云数据
    io::ReadPointCloudFromPCD(param_.map_gt_path_, *gt_3d_, io_params);            //!读取gt-map点云数据
    if (map_3d_->IsEmpty() || gt_3d_->IsEmpty()) {               
        cout << "Empty point cloud detected" << endl;
        return -1;
    }
    //* Downsampling for efficiency（重要的参数)
    map_3d_ = map_3d_->VoxelDownSample(0.01);  //体素网格降采样
    gt_3d_ = gt_3d_->VoxelDownSample(0.01);
    t1 = tic_toc.toc();  //*统计加载点云和降采样时间
    ...
    if (param_.evaluate_mme_) {   //! 估计均值地图熵
        double radius = 0.2;         //kdtree的搜索半径值
        mme_est = ComputeMeanMapEntropy(map_3d_, est_entropies, radius);   //todo 计算LIO地图的均值和熵

        //!没有gt_map直接可以将下述代码注释调-----------------------------------------代测试------------------
        //*参考链接：https://www.zhihu.com/question/585951892/answer/3346129332
        //! -----------------------------------------------------------------------------------------------------------------------
        ...
        if (param_.evaluate_gt_mme_) {                //todo 计算GT-MAP地图的均值和熵
            mme_gt = ComputeMeanMapEntropy(gt_3d_, gt_entropies, radius);  //*核心函数1
            std::cout << "MME EST-GT: " << mme_est << " " << mme_gt << std::endl;
        } else {
            std::cout << "MME EST: " << mme_est << std::endl;
        }
        map_3d_entropy = ColorPointCloudByMME(map_3d_, est_entropies);  //*根据MME熵值给点云上色
    ...
    /**  if building mesh */             //! 创建mesh（没有使用)
    if (eva_mesh) {
        gt_mesh = createMeshFromPCD(gt_3d_, 0.6, 10);   //*利用pcd创建gt的mesh
    ...
    //todo-------------------------------------------------------------------------------------------------没有GT-MAP下述代码可以注释调
    // ICP registration                       //!可以考虑改为ndt配准(改动的地方)
    pipelines::registration::RegistrationResult registration_result;
    registration_result = performICPRegistration();    //* ICP配准（核心函数2）
    t4 = tic_toc.toc();
    std::cout << "4. aligned cloud : " << (t4 - t3) / 1000.0 << " [s]"
              << std::endl;
    std::cout << "aligned transformation: \n" << trans << std::endl;         //配准的变换结果

    if(param_.evaluation_method_==0 || param_.evaluation_method_==1 || param_.evaluation_method_==2){
        std::cout << "icp overlap ratio: " << registration_result.fitness_             //内点（source点云中的点落在target点云中）的重叠率，越大越好
              << std::endl;
        std::cout << "icp correspondences rmse: " << registration_result.inlier_rmse_    //内点的均方根误差，越小越好
                << std::endl;
        std::cout << "icp correspondences size: "
                << registration_result.correspondence_set_.size() << std::endl;   //source和target点云之间的相应
    }else{  //!---------------------------------------------------------------------------my_add------------------------------------
        std::cout << "ndt overlap ratio: " << registration_result.fitness_             //内点（source点云中的点落在target点云中）的重叠率，越大越好
              << std::endl;
        std::cout << "ndt correspondences rmse: " << registration_result.inlier_rmse_    //内点的均方根误差，越小越好
                << std::endl;
        std::cout << "ndt correspondences size: "
                << registration_result.correspondence_set_.size() << std::endl;   //source和target点云之间的相应
    } //!-------------------------------------------------------------------------------------------------my_add---------------------------------------------------
    

    //todo Calculate metrics and evaluate
    t5 = tic_toc.toc();
    calculateMetrics(registration_result);   //*核心函数3         //根据截断距离评估配准的点对
    t7 = tic_toc.toc();
...
std::shared_ptr<open3d::geometry::PointCloud> CloudMapEva::ColorPointCloudByMME(                   //todo 被调用
        const std::shared_ptr<open3d::geometry::PointCloud> &pointcloud,
        const std::vector<double> &entropies) {
...
//    colored_pointcloud->PaintUniformColor(Eigen::Vector3d(1, 1, 1));
    ...
    std::vector<double> non_zero_entropies;
    std::copy_if(entropies.begin(), entropies.end(), std::back_inserter(non_zero_entropies),
                 [](double entropy) { return entropy != 0.0; });   //保存非零熵值
    max_abs_entropy = std::fabs(*std::min_element(non_zero_entropies.begin(), non_zero_entropies.end()));  //熵值一般为负值，所以才这样操作
    min_abs_entropy = std::fabs(*std::max_element(non_zero_entropies.begin(), non_zero_entropies.end()));
    ...
    for (size_t i = 0; i < entropies.size(); ++i) {
        if (valid_entropy_points[i]) {
            ...
            double normalized_entropy =
                    (std::fabs(entropies[i]) - min_abs_entropy) / (max_abs_entropy - min_abs_entropy);            //归一化作者代码写的不好，直接算就完了（不就是计算比例吗)
...
void CloudMapEva::getDiffRegResult(                    //*被调用了
...
void CloudMapEva::getDiffRegResult(std::vector<Vector5d> &result,
    ...
    //*变量：存储配准点的个数、适配度、均值、RMSE 和标准差。这些向量的每个维度分别对应不同的截断距离 param_.trunc_dist_
    Vector5i number_vec = Vector5i::Zero();
    Vector5d fitness_vec = Vector5d::Zero();
    Vector5d mean_vec = Vector5d::Zero();
    Vector5d rmse_vec = Vector5d::Zero();
    ...
    // accuracy (mean) and precision (standard deviation)
    for (int i = 0; i < points_set.size(); ++i) {     
        Eigen::Vector2i points_i = points_set.at(i);                   //points_set存放的是点对索引 (source与target)
        auto map_pt = source.points_.at(points_i[0]);
        auto gt_pt = target.points_.at(points_i[1]);

        source_set.points_.push_back(map_pt);   //将对应的点存入点云中
        target_set.points_.push_back(gt_pt);

        double norm_dis = (map_pt - gt_pt).norm();   //计算对应点之间的欧式距离
        double squre_dis = (map_pt - gt_pt).squaredNorm();
        est_gt_dis.at(i) = norm_dis;
        ...
        if (norm_dis <= param_.trunc_dist_[1]) {    //10cm
            mean_vec[1] += norm_dis;
            rmse_vec[1] += squre_dis;
            number_vec[1]++;
        }
        if (norm_dis <= param_.trunc_dist_[2]) {    //5cm
            mean_vec[2] += norm_dis;
            rmse_vec[2] += squre_dis;
            number_vec[2]++;
        }
        if (norm_dis <= param_.trunc_dist_[3]) {   //2cm
            mean_vec[3] += norm_dis;
            rmse_vec[3] += squre_dis;
            number_vec[3]++;
        }
        if (norm_dis <= param_.trunc_dist_[4]) {    //1cm
            mean_vec[4] += norm_dis;
            rmse_vec[4] += squre_dis;
            number_vec[4]++;
        }
    ...
    rmse_vec /= points_set.size();
    int source_num = source.points_.size();
    for (int i = 0; i < param_.trunc_dist_.size(); ++i) {           //分别计算每个截断距离下的值
        fitness_vec[i] = number_vec[i] * 1.0 / source_num;           //不同截断距离下的重叠率
        rmse_vec[i] = sqrt(rmse_vec[i]);                                           //不同截断距离下的均方根误差
        ...
        sigma /= points_set.size();
        sigma_vec[i] = sqrt(sigma);                             //不同截断距离下的标准差
    }
...
void CloudMapEva::calculateMetrics(pipelines::registration::RegistrationResult &registration_result) {
    ...
    getDiffRegResult(est_gt_results,
                     ...
                     *map_3d_, *gt_3d_, *corresponding_cloud_est, *corresponding_cloud_gt);  //*note:  *map_3d_已经变换到gt坐标系下了，与初始*map_3d_不同
    ...
    auto gt_est_icp_results = pipelines::registration::EvaluateRegistration(*gt_3d_, *map_3d_,
                                                                            param_.icp_max_distance_);           //! 可以考虑放在ndt配准中
    getDiffRegResult(gt_est_results, gt_est_icp_results.correspondence_set_, *gt_3d_, *map_3d_);  
    cd_vec = est_gt_results.at(1) + gt_est_results.at(1);  //* Chamfer Distance  （倒角距离=est_gt的rmse + gt_est的rmse)
    for (int i = 0; i < param_.trunc_dist_.size(); ++i) {
        double pr = 2 * est_gt_results.at(2)[i] * gt_est_results.at(2)[i];
        double p_plus_r = est_gt_results.at(2)[i] + gt_est_results.at(2)[i];
        f1_vec[i] = pr / p_plus_r;  //* F1 Score  (f1=2*est_gt的fitness*gt_est的fitness / est_gt的fitness + gt_est的fitness )
    }
    std::cout << "Chamfer Distance: " << cd_vec.transpose() << std::endl;
    std::cout << "F1 Score: " << f1_vec.transpose() << std::endl;
    std::cout << "MME: " << mme_est << " " << mme_gt << std::endl;        //输出地图和GT-MAP的地图熵
}

void CloudMapEva::saveResults() {                     //!保存的文件
    ...
    file_result << param_.name_ << " " << time_stream.str() << std::endl;                //保存名字 时间
    file_result << param_.map_gt_path_ << std::endl;                                                       //GT-MAP的路径
    file_result << param_.evaluation_map_pcd_path_ + "map.pcd" << std::endl;                 //估计的路径
    
    file_result << std::fixed << std::setprecision(15) << "Inlier Mean: "                  //保留15位小数
    ...
    file_result << std::fixed << std::setprecision(15) << "Inlier Fitness: "
    ...
    file_result << std::fixed << setprecision(5) << "CD: " << cd_vec.transpose()                 //保留5位小数
                << std::endl;
    file_result << std::fixed << setprecision(5) << "F1: " << f1_vec.transpose()
                << std::endl;
    if (eva_mme) {
        file_result << std::fixed << setprecision(5) << "MME: " << mme_est << " " << mme_gt << " "          //熵值一般为负值，如果估计GT-MAP均值熵，后面的即为GT-MAP的最小均值和最大均值熵（反之为SLAM的地图均值熵)
                    << min_abs_entropy << " " << max_abs_entropy << std::endl;      //均值地图熵：SLAM均值地图熵, GT-MAP地图熵,  最小均值熵(最大均值熵的绝对值),  最大均值熵(最小均值熵的绝对值)
    }
    file_result << "Time: " << t1 / 1000.0 << " " << (t2 - t1) / 1000.0 << " "         //时间： t1:统计加载点云和降采样时间  t2-t1:输出加载点云语句时间(感觉无太大意义)
                << (t3 - t2) / 1000.0 << " " << (t4 - t3) / 1000.0 << " "                                 //t3-t2: 估计均值地图熵和创建mesh的时间  t4-t3:点云配准时间
                << (t5 - t4) / 1000.0 << " " << (t7 - t5) / 1000.0 << std::endl;                   //t5-t4:输出配准结果语句时间(感觉无意义)  t7-t5:计算配准结果指标的时间
    file_result.close();
    std::cout << "Results saved to " << result_file_path << std::endl;

    //* rendering distance map
    map_3d_render_inlier = renderDistanceOnPointCloud(
            corresponding_cloud_gt, corresponding_cloud_est, param_.trunc_dist_[0]);                //内点距离误差点云 （截断距离选择0.2m)
    map_3d_render_raw =
            renderDistanceOnPointCloud(gt_3d_, map_3d_, param_.trunc_dist_[0]);              //原始距离误差点云
    //    visualization::DrawGeometries({map_3d_render_inlier}, "mesh result");

    open3d::io::WritePointCloud(                                                                                                                 //保存原始渲染距离图
    ...
    open3d::io::WritePointCloud(                                                                                                                        
            param_.evaluation_map_pcd_path_ + "inlier_rendered_dis_map.pcd",             //保存内点渲染距离图
    ...
    if (param_.evaluate_mme_) {
        open3d::io::WritePointCloud(
                param_.evaluation_map_pcd_path_ + "map_entropy.pcd",                                       //保存均值地图熵（SLAM地图)
                *map_3d_entropy);
        ...
        if (param_.evaluate_gt_mme_) {
            open3d::io::WritePointCloud(
                    param_.evaluation_map_pcd_path_ + "gt_entropy.pcd",                                      //保存均值地图熵（GT-MAP)
                    *gt_3d_entropy);
            std::cout << "Save rendered entropy gt map to "
                      << param_.evaluation_map_pcd_path_ + "gt_entropy.pcd"
                      << std::endl;
            visualization::DrawGeometries({gt_3d_entropy}, "gt_map_entropy result");               //可视化GT-MAP均值地图熵
        }
        visualization::DrawGeometries({map_3d_entropy}, "map_entropy result");                  //可视化MAP地图熵
    }
    //std::cout << "Results saved to " << result_file_path << std::endl;
    ...
    // visualization::DrawGeometries({map_3d_render_raw}, "mesh result");   //原代码
    visualization::DrawGeometries({map_3d_render_raw}, "raw_rendered_error result");   
}

pipelines::registration::RegistrationResult CloudMapEva::performICPRegistration() {     //*被调用
    pipelines::registration::RegistrationResult registration_result;
    auto criteria = pipelines::registration::ICPConvergenceCriteria();

    //!-------------------------------------------------------------my-add-----------------------------------------------------------------------
    // Loading target scan of room.
    // pcl::PointCloud<pcl::PointXYZ>::Ptr target_cloud (new pcl::PointCloud<pcl::PointXYZ>());
    // target_cloud->resize(gt_3d_->points_.size());
    // for(size_t i=0;i<gt_3d_->points_.size();i++){
    //     target_cloud->points[i].x=gt_3d_->points_[i][0];
    //     target_cloud->points[i].y=gt_3d_->points_[i][1];
    //     target_cloud->points[i].z=gt_3d_->points_[i][2];
    // }
    // std::cout << "Loaded " << target_cloud->size () << " data points from first.pcd" << std::endl;

    // // Loading source scan of room.
    // pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_cloud_source (new pcl::PointCloud<pcl::PointXYZ>());
    // filtered_cloud_source->resize(map_3d_->points_.size());
    // for(size_t i=0;i<map_3d_->points_.size();i++){
    //     filtered_cloud_source->points[i].x=map_3d_->points_[i][0];
    //     filtered_cloud_source->points[i].y=map_3d_->points_[i][1];
    //     filtered_cloud_source->points[i].z=map_3d_->points_[i][2];
    // }
    // std::cout << "Loaded " << filtered_cloud_source->size () << " data points from first.pcd" << std::endl;

    // if (pcl::io::loadPCDFile<pcl::PointXYZ> ("../dataset/MCR_slow/MCR_slow_gt.pcd", *target_cloud) == -1)
    // {
    //     std::cout<<"Couldn't read file MCR_slow_gt.pcd "<<std::endl;
    // }
    // std::cout << "Loaded " << target_cloud->size () << " data points from MCR_slow_gt.pcd" << std::endl;

    // // Loading second scan of room from new perspective.
    // pcl::PointCloud<pcl::PointXYZ>::Ptr source_cloud (new pcl::PointCloud<pcl::PointXYZ>);
    // if (pcl::io::loadPCDFile<pcl::PointXYZ> ("../dataset/MCR_slow/map.pcd", *source_cloud) == -1)
    // {
    //     std::cout<<"Couldn't read file map.pcd "<<std::endl;
    // }
    // std::cout << "Loaded " << source_cloud->size () << " data points from map.pcd" << std::endl;

    // ///对 source_cloud近似体素网格滤波
    // // Filtering input scan to roughly 10% of original size to increase speed of registration.
    // pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_cloud_source (new pcl::PointCloud<pcl::PointXYZ>);
    // pcl::ApproximateVoxelGrid<pcl::PointXYZ> approximate_voxel_filter_source;
    // approximate_voxel_filter_source.setLeafSize (1.1, 1.1, 1.1); //0.2*0.2*0.2      //!----------------------------------------
    // approximate_voxel_filter_source.setInputCloud (source_cloud);
    // approximate_voxel_filter_source.filter (*filtered_cloud_source);
    // std::cout << "source Filtered cloud contains " << filtered_cloud_source->size ()
    //         << " data points from map.pcd" << std::endl;
    ...
    switch (param_.evaluation_method_) {
        ...
        //!------------------------------------------自己加的----------------------------------------------ndt配准（这块没毛病，主要是后面点相应对无法获取，导致后续无法评估）
        case 3:  //NDT
            {
                // Loading target scan of room.
                pcl::PointCloud<pcl::PointXYZ>::Ptr target_cloud (new pcl::PointCloud<pcl::PointXYZ>());
                ...
                init_guess=param_.initial_matrix_.cast<float>();

                ndt.align (*output_cloud, init_guess);

                if(ndt.hasConverged ()){
                    std::cout << "Normal Distributions Transform has converged,score:  " << ndt.getFitnessScore () << std::endl;
                    trans = ndt.getFinalTransformation().cast<double>();
                    *map_3d_ = map_3d_->Transform(trans); 
                    registration_result = pipelines::registration::EvaluateRegistration(*gt_3d_, *map_3d_,param_.icp_max_distance_); 
                    return registration_result;
                }else{
                std::cout<<"NDT registration failed"<<std::endl;
                }
           }
            break;
        //!--------------------------------------------------------------------------------------------------------
    ...
    *map_3d_ = map_3d_->Transform(trans);      //*变换到gt坐标系下了
    return registration_result;
}

double CloudMapEva::ComputeEntropy(const Eigen::Matrix3d &covariance) {
    double entropy = 0.5 * std::log(2 * M_PI * M_E * covariance.determinant());   //*熵值计算公式
    return entropy;
}

double
CloudMapEva::ComputeMeanMapEntropy(const std::shared_ptr<open3d::geometry::PointCloud> &pointcloud,
                                   std::vector<double> &entropies, double radius) {  //*计算均值地图熵
    mean_entropy = 0.0;
    valid_points = 0;
    ...
    open3d::geometry::KDTreeFlann kdtree;
    kdtree.SetGeometry(*pointcloud);   //创建kd树
...
#pragma omp parallel for reduction(+:mean_entropy, valid_points)     //OMP并行化操作累计均值熵和有效点
    for (int i = 0; i < static_cast<int>(pointcloud->points_.size()); ++i) {
        std::vector<int> indices;
        std::vector<double> distances;
        if (kdtree.SearchRadius(pointcloud->points_[i], radius, indices, distances) > 0) {  //距离从小到大排列（kdtree半径搜索)
            // Remove the first index since it is the point itself(去除自身点)
            indices.erase(indices.begin());                          
            distances.erase(distances.begin());
            if (indices.size() < 15) {           
                // Not enough points to compute covariance matrix（点数不够无法计算协方差)
                continue;
            }
            Eigen::MatrixXd points(3, indices.size());   //构建矩阵
            for (size_t j = 0; j < indices.size(); ++j) {
                points.col(j) = pointcloud->points_[indices[j]].cast<double>();   //查找的点按列放
            }
            Eigen::Vector3d mean = points.rowwise().mean();               //计算每行的均值（即所有点累加起来的x,y,z均值）
            Eigen::MatrixXd centered = points.colwise() - mean;
            Eigen::Matrix3d covariance = (centered * centered.transpose()) / static_cast<double>(indices.size() - 1);   //协方差矩阵
            double entropy = ComputeEntropy(covariance);  //根据协方差矩阵计算熵
            //std::cout << "covariance determinant : " << covariance.determinant() << std::endl;
//            std::cout << "entropy: " << entropy << std::endl;
            if (!std::isnan(entropy) && !std::isinf(entropy)) {   //熵值非nan和非无限就累加
                mean_entropy += entropy;
                entropies[i] = entropy;
                valid_entropy_points[i] = true;
                ++valid_points;
        ...
        int current_processed = ++processed_points;  //processed_points初始值为9           //这段主要是进度条报告
...
#pragma omp critical
    ...
    if (valid_points > 0) {
        mean_entropy /= valid_points;           //*计算平均熵
    }
